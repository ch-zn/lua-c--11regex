...
const char regex_tname[]="std::regex";
...
namespace regex{
	int __new(lua_State *L){	// regex(string regex, regex_constants/uint flag)
		...
		if(lua_isinteger(L,3))flag=(syntax_option_type)(lua_tointeger(L,3));

		void* ptr=lua_newuserdatauv(L,sizeof(std::regex),1);
		try{
			new(ptr)std::regex(s,flag);
	...
	int __gc(lua_State *L){
		((std::regex*)lua_touserdata(L,1))->std::regex::~regex();
		return 0;
	}
	...
	static const luaL_Reg __index[]={
			...
			{"flags",flags},
			{"match",regex_algorithm::match},
			{"search",regex_algorithm::search},
			{"replace",regex_algorithm::replace},
			{"iterator",::regex_iterator::new_iterator},
			{nullptr,nullptr}
	};
