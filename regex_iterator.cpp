...
namespace lregex_iterator{
	...
	int iterator_func(lua_State *L){
		...
		auto cm=(cmatch*)luaL_checkudata(L,2,match_results_tname);
		lua_getiuservalue(L,2,1);
		size_t slen;
		const char *s=lua_tolstring(L,-1,&slen);
		lmatch_results::__new(L);
		auto cm2=(cmatch*)lua_touserdata(L,-1);
		lua_rotate(L,-2,1);
		lua_setiuservalue(L,-2,1);

		auto flag=(regex_constants::match_flag_type)lua_tointeger(L,lua_upvalueindex(1));
		...
		if(regex_search(
				...
				!cm->empty()
#endif
				?cm->operator[](0).second:s,s+slen,*cm2,*r,flag))return 1;
		else {
			lua_pop(L,1);
			return 0;
		}
